...
-- |  -- version : V1.3 for PostgreSQL 13                                                                        |
...
\qecho color:green; } 
\qecho </style> 
\qecho <h1 align="center" style="background-color:#e59003" >PG COLLECTOR  V1.3 for PostgreSQL 13</h1>
...
select count(*) > 0 isaurora from pg_settings where name='rds.extensions' and setting like '%aurora_stat_utils%' \gset
\if :isaurora 
select count(*) > 0 isaurora1311plus FROM aurora_version() where replace(substring(aurora_version, 1, 5),'.','')::int >= '1311' \gset
\if :isaurora1311plus
\qecho <br>
...
\else
    \if yes
        \qecho 'aurora_stat_memctx_usage function is only available staring from Aurora PostgreSQL version 13.11'
    \endif
\endif
\else
    \if yes
...
select count(*) > 0 isaurora from pg_settings where name='rds.extensions' and setting like '%aurora_stat_utils%' \gset
\if :isaurora
select count(*) > 0 isaurora138plus FROM aurora_version() where replace(substring(aurora_version, 1, 5),'.','')::int >= '138' \gset
\if :isaurora138plus
SELECT * FROM aurora_stat_logical_wal_cache();
\else
    \if yes
        \qecho 'aurora_stat_logical_wal_cache function is only available staring from Aurora PostgreSQL version 13.8'
    \endif
\endif
\else
    \if yes
